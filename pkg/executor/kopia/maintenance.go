...
func newMaintenanceCommand() *cobra.Command {
	...
	maintenanceCommand.Flags().StringVar(&maintenanceType, "maintenanc-type", "", "full - will run full maintenance and quick - will run quick maintenance")
...
func updateBackupLocationMaintenace(
	...
	if maintenanceType == fullMaintenanceType {
		if backupLocationMaintenance.Status.FullMaintenacneRepoStatus == nil {
			backupLocationMaintenance.Status.FullMaintenacneRepoStatus = make(map[string]kdmp_api.RepoMaintenanceStatus)
		}
		backupLocationMaintenance.Status.FullMaintenacneRepoStatus[repoName] = repoStatus
	} else {
		if backupLocationMaintenance.Status.QuickMaintenacneRepoStatus == nil {
			backupLocationMaintenance.Status.QuickMaintenacneRepoStatus = make(map[string]kdmp_api.RepoMaintenanceStatus)
		}
		backupLocationMaintenance.Status.QuickMaintenacneRepoStatus[repoName] = repoStatus
	}
...
func runMaintenance(maintenanceType string) error {
	...
	for _, repoName := range repoList {
		...
		if err := runKopiaRepositoryConnect(repo); err != nil {
			...
			if statusErr != nil {
				logrus.Warnf("update of status for repo [%v] failed: %v", repo.Name, statusErr)
			}
